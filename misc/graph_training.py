...
# python misc\graph_training.py data\logs\2018-08-28-00-04-11\2018-08-28-00-04-11_train.csv
...
def get_training_data(path: str) -> (np.ndarray, np.ndarray):
...
def main():
	...
	loss, val_loss = get_training_data(csv_file_path)

	if loss is None or len(loss) <= 0 or val_loss is None or len(val_loss) <= 0:
		return
	...
	plt.text(x_max / 2.0, y_max + 5.0, "Pérdida a lo largo del tiempo de entrenamiento", fontsize=16, weight='bold', ha="center")
