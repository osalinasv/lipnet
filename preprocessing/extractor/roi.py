def video_to_frames_roi(input_loc, output_loc_frame, output_loc_roi, shape_predictor):
    ...
    while cap.isOpened():
        ...
        extract_mouth(output_frame, output_roi, shape_predictor)
        count = count + 1
...
def extract_mouth(image, output_path_image, shape_predictor):
    OFFSET_MOUTH_Y = 15
    OFFSET_MOUTH_X = 15
    ...
    with_lipnext = True
    if with_lipnext:
        mouth = get_frames_mouth(detector, predictor, image)
    ...
    for (i, rect) in enumerate(rects):
        ...
        roi = lipnet_mouth_extraction(np_mouth_points[0], image)
