...
from preprocessing.extractor.extract_roi_to_npy import video_to_frames


# python preprocessing\extract_to_npy.py -v D:\GRID\ -o data\dataset -p bbaf*.mpg
def extract_to_npy(videos_path: str, output_path: str, predictor_path: str, pattern: str, first_video: int, last_video: int):
	...
	videos_already_processed = []

	VIDEOS_SUCCESS_PATH = os.path.join(output_path, 'videos_success.log')
	VIDEOS_FAILED_PATH  = os.path.join(output_path, 'videos_failed.log')

	if is_file(VIDEOS_SUCCESS_PATH):
		with open(VIDEOS_SUCCESS_PATH, 'r+') as f:
			for line in f.readlines():
				videos_already_processed.append(line.rstrip())
	...
	for file_path in get_files_in_dir(videos_path, pattern):
		...
		if first_video < video_count_per_group > last_video:
			continue
		...
		if video_target_path in videos_already_processed:
			print('Video {} was already extracted to: {}'.format(video_file_name, video_target_path))
			continue

		make_dir_if_not_exists(video_target_dir)

		if video_to_frames(file_path, video_target_path, predictor_path):
			with open(VIDEOS_SUCCESS_PATH, 'a+') as f:
...
def main():
	...
	ap.add_argument("-lv", "--last-video",  required=False,
		help="(Optional) Last video extracted in each speaker (inclusive)", type=int, default=1001)
