...
class Lipnext(object):
	...
	def __init__(self, frame_count: int, image_channels: int, image_height: int, image_width: int, max_string: int, output_size: int = env.OUTPUT_SIZE):
		...
		self.drop_1 = layers.create_drop_layer('drop_1', self.actv_1)
		self.pool_1 = layers.create_pool_layer('pool_1', self.drop_1)

		self.zero_2 = layers.create_zero_layer('zero_2', self.pool_1)
		...
		self.drop_2 = layers.create_drop_layer('drop_2', self.actv_2)
		self.pool_2 = layers.create_pool_layer('pool_2', self.drop_2)

		self.zero_3 = layers.create_zero_layer('zero_3', self.pool_2, padding=(1, 1, 1))
		...
		self.drop_3 = layers.create_drop_layer('drop_3', self.actv_3)
		self.pool_3 = layers.create_pool_layer('pool_3', self.drop_3)

		self.res = layers.create_timed_layer(self.pool_3)
