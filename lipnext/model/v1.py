...
class LipNext(object):
	...
	def __init__(self, *, frame_count: int, image_channels: int, image_height: int, image_width: int, max_string: int, output_size: int = OUTPUT_SIZE):
	...
	def compile(self):
		adam = Adam(lr=LipNext.ADAM_LEARN_RATE, beta_1=LipNext.ADAM_F_MOMENTUM, beta_2=LipNext.ADAM_S_MOMENTUM, epsilon=LipNext.ADAM_STABILITY)
		self.model.compile(loss={'ctc': lambda y_true, y_pred: y_pred}, optimizer=adam)

	def load_weights(self, path: str):
		self.model.load_weights(path)

	def train(self, *, generator, validator=None, start_epoch: int = 0, stop_epoch: int = 1, training_steps: int = None, validation_steps: int = None, callbacks: list = None):
	...
	def create_conv(name: str, input_layer, filters: int, kernel_size: tuple = CONV_KERNEL_SIZE) -> Conv3D:
		return Conv3D(filters, kernel_size, strides=LipNext.CONV_STRIDES, kernel_initializer=LipNext.CONV_KERNEL_INIT, name=name)(input_layer)
	...
	def create_bi_gru(name: str, input_layer, units: int = 256) -> Bidirectional:
		return Bidirectional(GRU(units, return_sequences=True, kernel_initializer=LipNext.GRU_KERNEL_INIT, name=name), merge_mode='concat')(input_layer)
