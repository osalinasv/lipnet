...
def generate_train_val(mouths_path, destination_path, train_size, val_size):
    """
    Copy the mouths folders to the destination path and separates it into train a val folders
    depending on the train size and val size
    :param mouths_path:
    ...
    for subdir, dirs, files in walklevel(mouths_path):
        train_count = 0
        val_count = 0
        ...
        for subdir_2, dirs_2, files_2 in walklevel(subdir):
            if subdir_2 == subdir:
                continue
